...
class AdminSidebar
{
    public static function build(?User $adminUser, bool $isDemoDashboard, int $newOrdersCount): array
    {
        ...
        $featureEnabled = static fn (string $feature): bool => $subscription->featureEnabled($feature);
        ...
        $dashboardLinks = [];
        if ($isDeliveryUser) {
            if ($featureEnabled('delivery_dashboard')) {
                $dashboardLinks[] = [
            ...
            if ($featureEnabled('kitchen_dashboard') && ($adminUser?->isSuperAdmin() || $adminUser?->isOwner() || $adminUser?->role === User::ROLE_MANAGER || $adminUser?->role === User::ROLE_KITCHEN)) {
                $dashboardLinks[] = [
                    'label' => 'شاشة المطبخ',
            ...
            if ($featureEnabled('cashier_pos') && $adminUser?->hasPermission('use_cashier') && $adminUser?->branch_id) {
                $dashboardLinks[] = [
                    'label' => 'شاشة الكاشير',
            ...
            if ($featureEnabled('kitchen_dashboard')) {
                $dashboardLinks[] = [
                    'label' => 'شاشة المطبخ',
                    'url' => $demoOrAdminUrl('kitchen', route('admin.kitchen.index')),
                    'active' => request()->routeIs('admin.kitchen.*'),
            ...
            if ($featureEnabled('delivery_dashboard') && $hasAdminPermission('manage_delivery')) {
                $dashboardLinks[] = [
        ...
        if (!$isDeliveryUser && !$isKitchenUser) {
            if ($featureEnabled('branches') && $hasAdminPermission('manage_branches')) {
            ...
            if ($hasAdminPermission('manage_settings')) {
                ...
                if ($featureEnabled('theme_switching')) {
        ...
        $digitalMenuLinks = [];
        if (!$isDeliveryUser && !$isKitchenUser && $featureEnabled('online_menu') && $hasAdminPermission('manage_digital_menu') && Route::has('admin.digital-menu.settings')) {
            ...
            if ($featureEnabled('qr_menu') && Route::has('admin.digital-menu.qr')) {
        ...
        $adsLinks = [];
        if (!$isDeliveryUser && !$isKitchenUser && $featureEnabled('popup_campaigns') && Route::has('admin.popup-campaign.edit')) {
